...
COMMENT_PATTERN="//".*
IDENTIFIER_PATTERN=([_a-zA-Z\\][a-zA-Z0-9\_\\]*)
INTEGER_PATTERN=([\-]?[0-9]+)|([\-]?[0][x][0-9A-Fa-f]+)
DOUBLE_PATTERN=([\-]?[0-9]+[\.][0-9]+)
...
SCHAR_PATTERN = {SINGLE_QUOTE} (( [^'\\] | "\\" ( {SINGLE_QUOTE} | {COMMON_ESCAPE}) ) | [^\x20-\x7E]{1,2}) {SINGLE_QUOTE}
STRING_PATTERN = {DOUBLE_QUOTE} ( [^\"\\] | "\\" ( {DOUBLE_QUOTE} | {SINGLE_QUOTE} | {COMMON_ESCAPE}) )* {DOUBLE_QUOTE}
//CBLOCK="//"(\"%{\"([^}]+|[}]+[^%{])*\"}%\")
...
<YYINITIAL> {
  ...
  "var"              { yybegin(YYINITIAL); return ZephirTypes.VAR; }
  "void"             { yybegin(YYINITIAL); return ZephirTypes.VOID; }
  "int"              { yybegin(YYINITIAL); return ZephirTypes.INT; }
  "uint"             { yybegin(YYINITIAL); return ZephirTypes.UINT; }
  "long"             { yybegin(YYINITIAL); return ZephirTypes.LONG; }
  "ulong"            { yybegin(YYINITIAL); return ZephirTypes.ULONG; }
  "char"             { yybegin(YYINITIAL); return ZephirTypes.CHAR; }
  "uchar"            { yybegin(YYINITIAL); return ZephirTypes.UCHAR; }
  "double"           { yybegin(YYINITIAL); return ZephirTypes.DOUBLE; }
  "float"            { yybegin(YYINITIAL); return ZephirTypes.DOUBLE; }
  "bool"             { yybegin(YYINITIAL); return ZephirTypes.BOOL; }
  "boolean"          { yybegin(YYINITIAL); return ZephirTypes.BOOL; }
  "string"           { yybegin(YYINITIAL); return ZephirTypes.STRING; }
  "array"            { yybegin(YYINITIAL); return ZephirTypes.ARRAY; }
  "object"           { yybegin(YYINITIAL); return ZephirTypes.OBJECT; }
  "callable"         { yybegin(YYINITIAL); return ZephirTypes.CALLABLE; }
  "resource"         { yybegin(YYINITIAL); return ZephirTypes.RESOURCE; }
  ...
  {COMMENT_PATTERN}          { yybegin(YYINITIAL); return ZephirTypes.COMMENT_PATTERN; }
  {IDENTIFIER_PATTERN}       { yybegin(YYINITIAL); return ZephirTypes.IDENTIFIER_PATTERN; }
  {INTEGER_PATTERN}          { yybegin(YYINITIAL); return ZephirTypes.INTEGER_PATTERN; }
  {DOUBLE_PATTERN}           { yybegin(YYINITIAL); return ZephirTypes.DOUBLE_PATTERN; }
  {SCHAR_PATTERN}            { yybegin(YYINITIAL); return ZephirTypes.SCHAR_PATTERN; }
  {STRING_PATTERN}           { yybegin(YYINITIAL); return ZephirTypes.STRING_PATTERN; }
//  {CBLOCK}           { yybegin(YYINITIAL); return ZephirTypes.CBLOCK_PATTERN; }
